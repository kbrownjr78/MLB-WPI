...
class MLBInningByInningEngine:
    ...
    def _fetch_fallback_lineup(self, team_id):
        """Helper tool to resolve the top active starting players using roster maps."""
        try:
            roster = statsapi.get_team_roster(team_id)
            # Filter strictly for positional fielders, mapping their full names out
            batters = [p['fullName'] for p in roster if p['position'] not in ['Pitcher', 'Two-Way Player']]
            return batters[:9]
        except Exception:
            return [f"Lineup Fielder {i}" for i in range(1, 10)]

    def fetch_live_schedule(self):
        """Ingests current day games along with prioritized starting lineups directly from MLB StatsAPI."""
        try:
            ...
            for g in raw_games:
                ...
                # Tier 1 Validation: Attempt to pull active live game starting cards
                try:
                ...
                # Tier 2 Validation: Fall back to official active rosters if boxscore isn't generated yet
                if not away_batters and g.get('away_id'):
                    away_batters = self._fetch_fallback_lineup(g['away_id'])
                if not home_batters and g.get('home_id'):
                    home_batters = self._fetch_fallback_lineup(g['home_id'])
    ...
    def calculate_custom_engine_metrics(self, game, db, delta_env):
        ...
        # 3. Match Verified Lineup Hitters by Name tokens
